...
const updateHomePageSettings = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { welcome_badge, hero_title, hero_description, background_value, contact_number, contact_email, website_title, website_logo, location, status } = req.body;
...
const createHomePageSettings = async (req, res, next) => {
  try {
    ...
    const createdResults = await Query(createdQuery, [result.insertId], [homePageModel.prefix_]);
