...
const getCategoryName = async (categoryId) => {
  ...
  try {
    ...
    const result = await Query(categoryQuery, [categoryId], [Master.master_category.prefix_]);
  ...
  try {
    ...
    if (media && media.length > 0 && postId) {
      ...
      for (let i = 0; i < media.length; i++) {
        ...
        await Query(mediaInsertQuery, [postId, mediaItem, i], [Master.post_media.prefix_]);
      }
...
const getPosts = async (req, res, next) => {
  try {
    ...
    const rawPosts = await Query(selectQuery, [parseInt(limit), (page - 1) * limit], [Master.post.prefix_, Master.master_category.prefix_]);
    ...
    const posts = await Promise.all(rawPosts.map(async (post) => {
      ...
      const mediaRows = await Query(mediaQuery, [post.id], [Master.post_media.prefix_]);
      const media = mediaRows.map(row => row.media).filter(item => item && typeof item === 'string');
...
const getPostById = async (req, res, next) => {
  try {
    ...
    const posts = await Query(selectQuery, [id], [Master.post.prefix_, Master.master_user.prefix_]);
    ...
    // Fetch media from post_media table
    const mediaQuery = `SELECT pm_media FROM ${Master.post_media.tablename} WHERE pm_post_id = ? ORDER BY pm_sort ASC`;
    const mediaRows = await Query(mediaQuery, [id], [Master.post_media.prefix_]);
    const media = mediaRows.map(row => row.media).filter(item => item && typeof item === 'string');
...
const getPublicFeed = async (req, res, next) => {
  try {
    ...
    const posts = await Promise.all(rawPosts.map(async (post) => {
      ...
      const likeResult = await Query(likeQuery, [post.id], [Master.post_like.prefix_]);
      ...
      const commentResult = await Query(commentQuery, [post.id], [Master.post_comment.prefix_]);
  ...
  try {
    ...
    if (media && Array.isArray(media)) {
      ...
      if (media.length > 0) {
        ...
        for (let i = 0; i < media.length; i++) {
          const mediaItem = media[i];
          await Query(mediaInsertQuery, [id, mediaItem, i], [Master.post_media.prefix_]);
        }

        console.log(`Updated media: deleted old, inserted ${media.length} new items for post ${id}`);
      }
    }
