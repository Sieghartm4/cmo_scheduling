
// THIS FILE IS AUTO-GENERATED by a script.
// Joi data types and error messages are automatically mapped for INSERT/UPDATE.

const Joi = require('joi');

const AppointmentSchema = Joi.object({
 	id: Joi.number().integer().required().messages({
        'any.required': 'id is required.',
        'number.base': 'id must be a valid bigint.',
      }),
 	muId: Joi.number().integer().required().messages({
        'any.required': 'muId is required.',
        'number.base': 'muId must be a valid bigint.',
      }),
 	date: Joi.string().isoDate().required().messages({
        'any.required': 'date is required.',
        'string.empty': 'date must be a valid dateonly.',
      }),
 	startTime: Joi.string().trim().required().messages({
        'any.required': 'startTime is required.',
        'string.empty': 'startTime must be a valid time.',
      }),
 	endTime: Joi.string().trim().required().messages({
        'any.required': 'endTime is required.',
        'string.empty': 'endTime must be a valid time.',
      }),
 	reason: Joi.string().trim().required().messages({
        'any.required': 'reason is required.',
        'string.empty': 'reason must be a valid text.',
      }),
 	status: Joi.string().trim().required().messages({
        'any.required': 'status is required.',
        'string.empty': 'status must be a valid enum.',
      }),
 	notes: Joi.string().trim().required().messages({
        'any.required': 'notes is required.',
        'string.empty': 'notes must be a valid text.',
      }),
 	createdAt: Joi.string().trim().required().messages({
        'any.required': 'createdAt is required.',
        'string.empty': 'createdAt must be a valid datetime.',
      })
});


/** Field references for clean, type-safe access */
const AppointmentField = {
    Id: 'id',
    MuId: 'mu_id',
    Date: 'date',
    StartTime: 'start_time',
    EndTime: 'end_time',
    Reason: 'reason',
    Status: 'status',
    Notes: 'notes',
    CreatedAt: 'created_at',
    All: ['id', 'mu_id', 'date', 'start_time', 'end_time', 'reason', 'status', 'notes', 'created_at'],
};


module.exports = {
  AppointmentSchema,
  AppointmentField, 
};
