    ...
    if (window.confirm(`Are you sure you want to delete user "${row.fullname}"?`)) {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_LINK}/api/users/${row.id}`,
          {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${adminToken}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bodyData = {
        ...
        mu_role: formData.role,
        mu_profile: formData.profile,
        mu_status: formData.status,
      }

      // Only include password for new users
      if (!editingUser) {
        bodyData.mu_password = formData.password
      }
          ...
          <div className="space-y-4">
            ...
            <div className="border-b border-gray-200 pb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-gray-500">ID: #{viewingUser.id}</span>
              </div>
            </div>
