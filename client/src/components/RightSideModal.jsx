...
function RightSideModal({ isOpen, onClose, title, children, size = 'md' }) {
  ...
  return (
    <>
      ...
      <div className={`fixed right-0 top-0 h-full w-full ${sizeClasses[size]} bg-white shadow-[-10px_0_30px_rgba(0,0,0,0.2)] z-[90] transform transition-transform duration-300 ease-in-out animate-in slide-in-from-right flex flex-col`}>
        
        {/* Header - Green with Black accents */}
        <div className="bg-emerald-600 px-6 py-5 flex items-center justify-between relative flex-shrink-0">
          {/* Black line at the bottom of header */}
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-emerald-800 text-gray-400 hover:text-white transition-all duration-200"
          >
